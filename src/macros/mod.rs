...
///
///     * Or, `.sqlx` must exist in the crate's own directory or at the workspace root.
///       See [Offline Mode](#offline-mode-requires-the-offline-feature) below.
///
...
/// again.
///
/// In a workspace, `cargo sqlx prepare --workspace` writes a single `.sqlx` directory at the
/// workspace root. Alternatively, `cargo sqlx prepare --per-crate` gives each crate in the
/// workspace its own `.sqlx` directory next to its `Cargo.toml`, so that a change to one crate's
/// queries only touches that crate's query data. The macros look for a query in the crate's own
/// `.sqlx` first and fall back to the one at the workspace root, so either layout works
/// (and a crate may use both).
///
